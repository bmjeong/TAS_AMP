...
class Agent:
    def __init__(self, id_a, setting, build_type, conf_res_type, refine_type):
        ...
        if max(self.task_reward) > 1:
            for i1, t1 in enumerate(self.task):
                self.value_a2t[i1] = np.exp(
                    (self.task_reward[t1] - self.cost_a2t[i1] / self.vel - self.task_time[t1] / self.base_dist)
                    / max(self.task_reward))
                ...
                for i2, t2 in enumerate(self.task):
                    ...
                    self.value_t2t[i1, i2] = np.exp(
                        (self.task_reward[i2] - self.cost_t2t[i1, i2] / self.vel - self.task_time[
                            t2] / self.base_dist) / max(self.task_reward))
    ...
    def cal_loss_remove_element(self, N_pck, route, id_loc, route_time, msg_ratio):
    ...
    def cal_loss_remove(self, N_pck, route, route_time, msg_ratio):
        value_a2t = self.value_a2t
        value_t2t = self.value_t2t
        ...
        for id_loc in range(N_pck):
            route_reward[id_loc] = self.task_reward[route[id_loc]] * np.exp(- self.discount * (route_time[id_loc] - self.task_time[route[id_loc]]))
            ...
            if N_pck > 1:
                if id_loc == 0:
                    loss_remove[id_loc] = (msg_ratio[id_t4a] * value_a2t[route[0]] * value_t2t[route[0], route[1]] /
                                           value_a2t[route[1]])
                    dt = (self.cost_a2t[route[0]] + self.cost_t2t[route[0], route[1]] - self.cost_a2t[route[1]]) / self.vel
                    ...
                    loss_remove[id_loc] = (msg_ratio[id_t4a] * value_t2t[route[id_loc - 1], route[id_loc]] *
                                           value_t2t[route[id_loc], route[id_loc + 1]] /
                                           value_t2t[route[id_loc - 1], route[id_loc + 1]])
                    dt = (self.cost_t2t[route[id_loc - 1], route[id_loc]] + self.cost_t2t[route[id_loc], route[id_loc + 1]] - self.cost_t2t[route[id_loc - 1], route[id_loc + 1]]) / self.vel
                if self.discount > 0:
                    # rr = 0
                    # for i in range(N_pck - id_loc - 1):
                    #     rr += self.task_reward[route[id_loc + i + 1]] * np.exp(- self.discount * (route_time[id_loc + i + 1] - self.task_time[route[id_loc + i + 1]]))

                    rr = sum(route_reward[id_loc + 1:N_pck])

                    if max(self.task_reward) > 1:
                        rr = rr / max(self.task_reward)
                    dt *= self.base_dist
                    if dt > 0:
                        dt += self.task_time[route[id_loc]]
                    if max(self.task_reward) > 1:
                        r_ = np.exp(self.task_reward[route[id_loc]] * (np.exp(- self.discount * (route_time[id_loc] - self.task_time[route[id_loc]])) - 1) / max(self.task_reward))
                    else:
                        r_ = np.exp(
                            self.task_reward[route[id_loc]] * (np.exp(- self.discount * (route_time[id_loc] - self.task_time[route[id_loc]])) - 1))
                    loss_remove[id_loc] *= r_ * np.exp((np.exp( - self.discount * dt) - 1) * rr)
                ...
                if self.discount > 0:
                    if max(self.task_reward) > 1:
                        ...
                        r_ = np.exp(
                            self.task_reward[route[id_loc]] * (np.exp(- self.discount * (route_time[id_loc] - self.task_time[route[id_loc]])) - 1))
                    loss_remove[id_loc] *= r_
        return loss_remove

    def cal_val_remove(self, N_pck, route, route_time, msg_ratio):
        value_a2t = self.value_a2t
        value_t2t = self.value_t2t
        ...
        if N_pck > 1:
            for id_loc in range(N_pck):
                ...
                mr = msg_ratio[id_t4a]
                if id_loc == 0:
                    val_remove[id_loc] = (mr * value_a2t[route[0]] *
                                          value_t2t[route[0], route[1]] / value_a2t[route[1]])
                elif id_loc == N_pck - 1:
                    val_remove[id_loc] = (mr * value_t2t[route[-2], route[-1]])
                else:
                    val_remove[id_loc] = (value_t2t[route[id_loc - 1], route[id_loc]] *
                                          value_t2t[route[id_loc], route[id_loc + 1]] /
                                          value_t2t[route[id_loc - 1], route[id_loc + 1]] * mr)
        else:
            val_remove[0] = msg_ratio[route[0]] * value_a2t[route[0]]
        return val_remove
    ...
    def prune_all_at_once(self, msg_ratio):
        ...
        loss_remove = self.cal_loss_remove(N_pck, route, route_time, msg_ratio)
        #
        # loss_remove = self.cal_val_remove(N_pck, route, route_time, msg_ratio)

        # print(N_pck, len(route), route)

        # loss_remove = np.zeros(N_pck)
        # for i in range(N_pck):
        #     loss_remove[i] = self.cal_val_remove(N_pck, route, route_time, msg_ratio[i], i)
        ...
        for i in range(len(route)):
            if loss_remove[i] <= 1:
                ...
                # rem_route_time.append(route_time[i])

        for r in rem_route:
            route.remove(r)

        # for r in rem_route_time:
        #     route_time.remove(r)
    ...
    def insert_first(self, msg_ratio):
        # benefit = msg_ratio * self.value_a2t
        if self.discount > 0:
            benefit = np.zeros(self.N_task)
            for i in range(self.N_task):
                benefit[i] = msg_ratio[i] * self.value_a2t[i]
                # r_ = np.exp(self.task_reward[i] * (np.exp(- self.discount * self.cost_a2t[i] * self.base_dist) - 1))
                if max(self.task_reward) > 1:
                    r_ = np.exp(
                        self.task_reward[i] * (np.exp(- self.discount * self.cost_a2t[i] * self.base_dist) - 1) / max(
                            self.task_reward))
                else:
                    r_ = np.exp(
                        self.task_reward[i] * (np.exp(- self.discount * self.cost_a2t[i] * self.base_dist) - 1))
                benefit[i] *= r_
                # if self.id == 0 and i == 0:
                #     print(i, np.log(self.value_a2t[i]), r_, benefit[i], msg_ratio[i])
        else:
            benefit = msg_ratio * self.value_a2t
        max_id = np.argmax(benefit)
        if benefit[max_id] / msg_ratio[max_id] > 1 and benefit[max_id] > 1:
            self.route = [int(max_id)]
            self.Cal_route_time()
            self.decision[max_id] = 1
            self.route_length = 1
    ...
    def insert_task(self, msg_ratio):
        ...
        for task, mr in zip(rem_task, rem_msg_ratio):
            ...
            # print(mr)
            if self.discount > 0:
                dt = (self.cost_a2t[task] + self.cost_t2t[task, route[0]] - self.cost_a2t[route[0]]) / self.vel * self.base_dist
                dt += self.task_time[task]
                tn = self.cost_a2t[task] / self.vel * self.base_dist
                # r_ = np.exp(self.task_reward[task] * (np.exp(- self.discount * tn) - 1))
                if max(self.task_reward) > 1:
                    r_ = np.exp(self.task_reward[task] * (np.exp(- self.discount * tn) - 1) / max(self.task_reward))
                else:
                    r_ = np.exp(self.task_reward[task] * (np.exp(- self.discount * tn) - 1))
                # rr = 0
                # for i in range(N_pck):
                #     rr += self.task_reward[route[i]] * np.exp(- self.discount * (route_time[i] - self.task_time[route[i]]))

                value_insert[0, task] *= r_ * np.exp((np.exp(- self.discount * dt) - 1) * rr)

            value_insert[N_pck - 1, task] = value_t2t[route[-1], task] * mr
            if self.discount > 0:
                tn = route_time[-1] + (self.cost_t2t[route[-1], task]) / self.vel * self.base_dist
                if max(self.task_reward) > 1:
                    r_ = np.exp(self.task_reward[task] * (np.exp(- self.discount * tn) - 1) / max(self.task_reward))
                else:
                    r_ = np.exp(self.task_reward[task] * (np.exp(- self.discount * tn) - 1))
                value_insert[N_pck - 1, task] *= r_
            ...
            for id_loc in range(N_pck - 1):
                ...
                if self.discount > 0:
                    dt = (self.cost_t2t[route[id_loc], task] + self.cost_t2t[task, route[id_loc + 1]]
                          - self.cost_t2t[route[id_loc], route[id_loc + 1]]) / self.vel * self.base_dist
                    dt += self.task_time[task]
                    tn = route_time[id_loc] + (self.cost_t2t[route[id_loc], task]) / self.vel * self.base_dist
                    if max(self.task_reward) > 1:
                        r_ = np.exp(self.task_reward[task] * (np.exp(- self.discount * tn) - 1) / max(self.task_reward))
                    else:
                        r_ = np.exp(self.task_reward[task] * (np.exp(- self.discount * tn) - 1))
                    ...
                    if max(self.task_reward) > 1:
                        rr = rr / max(self.task_reward)
                    value_insert[id_loc + 1, task] *= r_ * np.exp((np.exp(- self.discount * dt) - 1) * rr)

        (loc, ta) = max(value_insert, key=value_insert.get)
        # print(self.id, self.iter_cnt, self.route, msg_ratio[ta])
        if value_insert[loc, ta] / msg_ratio[ta] > 1 and  value_insert[loc, ta] > 1 :
            # print(self.id, value_insert)
            N_pck = N_pck + 1
    ...
    def cal_msg1_remove(self, N_pck, route, route_time, id_loc, route_reward):
        value_a2t = self.value_a2t
        value_t2t = self.value_t2t
        if N_pck > 1:
            if id_loc == 0:
                msg_1 = value_a2t[route[0]] * value_t2t[route[0], route[1]] / value_a2t[route[1]]
                dt = (self.cost_a2t[route[0]] + self.cost_t2t[route[0], route[1]] - self.cost_a2t[route[1]]) / self.vel
            elif id_loc == N_pck - 1:
                msg_1 = value_t2t[route[-2], route[-1]]
                dt = 0
            else:
                msg_1 = ((value_t2t[route[id_loc - 1], route[id_loc]] *
                                  value_t2t[route[id_loc], route[id_loc + 1]]) /
                                 value_t2t[route[id_loc - 1], route[id_loc + 1]])
                dt = (self.cost_t2t[route[id_loc - 1], route[id_loc]] + self.cost_t2t[
                    route[id_loc], route[id_loc + 1]] - self.cost_t2t[route[id_loc - 1], route[id_loc + 1]]) / self.vel
            if self.discount > 0:
                # rr = 0
                # for i in range(N_pck - id_loc - 1):
                #     rr += self.task_reward[route[id_loc + i + 1]] * np.exp(- self.discount * (route_time[id_loc+ i + 1] - self.task_time[route[id_loc + i + 1]]))
                ...
                if dt > 0:
                    dt += self.task_time[route[id_loc]]
                # r_ = np.exp(self.task_reward[route[id_loc]] * (np.exp(- self.discount * route_time[id_loc]) - 1))
                if max(self.task_reward) > 1:
                    r_ = np.exp(self.task_reward[route[id_loc]] * (np.exp(- self.discount * (route_time[id_loc] - self.task_time[route[id_loc]])) - 1) / max(self.task_reward))
                else:
                    r_ = np.exp(self.task_reward[route[id_loc]] * (np.exp(- self.discount * (route_time[id_loc] - self.task_time[route[id_loc]])) - 1))
                msg_1 *= r_ * np.exp((np.exp( -self.discount * dt) - 1) * rr)
        else:
            msg_1 = value_a2t[route[0]]
            if self.discount > 0:
                # r_ = np.exp(self.task_reward[route[id_loc]] * (np.exp(- self.discount * route_time[id_loc]) - 1))
                if max(self.task_reward) > 1:
                    r_ = np.exp(self.task_reward[route[id_loc]] * (np.exp(- self.discount * (route_time[id_loc] - self.task_time[route[id_loc]])) - 1) / max(self.task_reward))
    ...
    def cal_msg1_insert(self, N_pck, route, route_time, id_t4a, route_reward):
        value_a2t = self.value_a2t
        value_t2t = self.value_t2t
        if N_pck > 0:
            ...
            if self.discount > 0:
                dt = (self.cost_a2t[id_t4a] + self.cost_t2t[id_t4a, route[0]] - self.cost_a2t[
                    route[0]]) / self.vel * self.base_dist
                dt += self.task_time[id_t4a]
                tn = self.cost_a2t[id_t4a] / self.vel * self.base_dist
                # r_ = np.exp(self.task_reward[id_t4a] * (np.exp(- self.discount * tn) - 1))
                if max(self.task_reward) > 1:
                    r_ = np.exp(self.task_reward[id_t4a] * (np.exp(- self.discount * tn) - 1) / max(self.task_reward))
                else:
                    r_ = np.exp(self.task_reward[id_t4a] * (np.exp(- self.discount * tn) - 1))
                # rr = 0
                # for i in range(N_pck):
                ...
                if max(self.task_reward) > 1:
                    rr = rr / max(self.task_reward)
                value_insert[0] *= r_ * np.exp((np.exp(- self.discount * dt) - 1) * rr)

            value_insert[-1] = value_t2t[route[-1], id_t4a]
            if self.discount > 0:
                tn = route_time[-1] + (self.cost_t2t[route[-1], id_t4a]) / self.vel * self.base_dist
                # r_ = np.exp(self.task_reward[id_t4a] * (np.exp(- self.discount * tn) - 1))
                if max(self.task_reward) > 1:
                    r_ = np.exp(self.task_reward[id_t4a] * (np.exp(- self.discount * tn) - 1) / max(self.task_reward))
                else:
            ...
            for id_loc in range(N_pck - 1):
                value_insert[id_loc + 1] = (value_t2t[route[id_loc], id_t4a] *
                                        ...
                                        value_t2t[route[id_loc], route[id_loc + 1]])
                if self.discount > 0:
                    dt = (self.cost_t2t[route[id_loc], id_t4a] + self.cost_t2t[id_t4a, route[id_loc + 1]]
                          - self.cost_t2t[route[id_loc], route[id_loc + 1]]) / self.vel * self.base_dist
                    # dt = 0
                    dt += self.task_time[id_t4a]
                    tn = route_time[id_loc] + (self.cost_t2t[route[id_loc], id_t4a]) / self.vel * self.base_dist
                    aa = (np.exp(- self.discount * tn) - 1)
                    r_ = np.exp(self.task_reward[id_t4a] * aa)
                    # print(r_)
                    rr = sum(route_reward[id_loc + 1:N_pck])
                    # rr = 0
                    # for i in range(N_pck - id_loc - 1):
                    #     rr += self.task_reward[route[id_loc + i + 1]] * np.exp(- self.discount * route_time[id_loc + i + 1])
                    value_insert[id_loc + 1] *= r_ * np.exp((np.exp(- self.discount * dt) - 1) * rr)
            msg_1 = max(value_insert)
        else:
            msg_1 = value_a2t[id_t4a]
            # print(msg_1)

            if self.discount > 0:
                # r_ = np.exp(self.task_reward[id_t4a] * (np.exp(- self.discount * self.cost_a2t[id_t4a] * self.base_dist / self.vel) - 1))
                if max(self.task_reward) > 1:
                    r_ = np.exp(self.task_reward[id_t4a] * (np.exp(- self.discount * self.cost_a2t[id_t4a] * self.base_dist / self.vel) - 1) / max(self.task_reward))
                else:
                    r_ = np.exp(self.task_reward[id_t4a] * (np.exp(- self.discount * self.cost_a2t[id_t4a] * self.base_dist / self.vel) - 1))
                msg_1 *= r_
                # print((np.exp(- self.discount * self.cost_a2t[id_t4a] * self.base_dist / self.vel) - 1))
        return msg_1
    ...
    def compute_msg_a2t(self):
        ...
        for id_t4a in range(self.N_task):
            if self.decision[id_t4a] == 1:  # msg_1 = 1
                ...
                msg_1[id_t4a] = self.cal_msg1_remove(N_pck, route, route_time, id_loc, route_reward)

            else:  # msg_0 = 1
                msg_1[id_t4a] = self.cal_msg1_insert(N_pck, route, route_time, id_t4a, route_reward)
                # print(msg_1[id_t4a])
    ...
    def dec_BP_msg(self, board, new_board):
        ...
        if np.max(comp_msg) < 1e-6:
            self.fin_cnt += 1
            # self.finish_flag = True
        else:
            self.fin_cnt = 0
            self.finish_flag = False
            # print(self.id, self.finish_flag)
    ...
    def cal_val_insert(self, N_pck, route, route_time, id_t4a):
        value_a2t = self.value_a2t
        value_t2t = self.value_t2t
        ...
        if self.discount > 0:
            dt = (self.cost_a2t[id_t4a] + self.cost_t2t[id_t4a, route[0]] - self.cost_a2t[route[0]]) / self.vel * self.base_dist
            dt += self.task_time[id_t4a]
            tn = self.cost_a2t[id_t4a] / self.vel * self.base_dist
            # r_ = np.exp(self.task_reward[id_t4a] * (np.exp(- self.discount * tn) - 1))
            if max(self.task_reward) > 1:
        ...
        for id_loc in range(N_pck - 2):
            value_insert[id_loc + 1] = value_t2t[route[id_loc], id_t4a] * value_t2t[
                id_t4a, route[id_loc + 1]] / value_t2t[route[id_loc], route[id_loc + 1]]
            if self.discount > 0:
                dt = (self.cost_t2t[route[id_loc], id_t4a] + self.cost_t2t[id_t4a, route[id_loc + 1]]
                      - self.cost_t2t[route[id_loc], route[id_loc + 1]]) / self.vel * self.base_dist
                dt += self.task_time[id_t4a]
                ...
                if max(self.task_reward) > 1:
                    ...
                    r_ = np.exp(self.task_reward[id_t4a] * (np.exp(- self.discount * tn) - 1))
                # rr = 0
                # for i in range(N_pck - id_loc - 2):
                #     rr += self.task_reward[route[id_loc + i + 1]] * np.exp(- self.discount * (route_time[id_loc + i + 1] - self.task_time[route[id_loc + i + 1]]))
                rr = sum(route_reward[id_loc:N_pck-1])
                if max(self.task_reward) > 1:
                    rr = rr / max(self.task_reward)
                value_insert[id_loc + 1] *= r_ * np.exp((np.exp(- self.discount * dt) - 1) * rr)
        ...
        if N_pck < self.cap:
            for id_t4a in range(self.N_task):
                ...
                if self.decision[id_t4a] == 0 and np.sum(board[id_t].msg[1, :]) == 0:
                    ...
                    # print(board[id_t].msg[0, max_id])
                    if board[id_t].agent[max_id] == id_a and board[id_t].msg[0, max_id] > 0.5:
                        route = self.route
                        route_time = self.route_time
                        N_pck += 1
                        if N_pck == 1:
                            # value_insert = self.cal_val_insert(N_pck, route, route_time, id_t4a)
                            ...
                            t_end_prev = route_time[-1]
                        self.decision[id_t4a] = 1
                        self.route = route
                        self.Cal_route_time()
                        self.route_length = N_pck

                        # rrr = self.task_reward[self.route[-1]] * np.exp(- self.discount * self.route_time[-1])
                        # ppp = self.route_time[-1] - t_end_prev
                        # print(rrr, ppp / self.base_dist)
                        # if rrr > ppp / self.base_dist:
                        if value_insert[max_loc] > 1:

                            # print(rrr, ppp, id_t4a, self.route_time)
                            self.two_opt()
        ...
        if self.iter_cnt < self.N_iter:
            if not self.finish_flag:
                ...
                if self.refine_type == 0:
                    if self.total_finish_flag and not self.refine_flag:
...
class AMP_CR:
    def __init__(self, prob, build_type=0, conf_res_type=0, refine_type=0, buff_size=2, discount=-1, N_iter=30):
        self.prob = prob
    ...
    def solve(self):
        ...
        for i in range(setting.N_agent):
            agent.append(Agent(i, setting, self.build_type, self.conf_res_type, self.refine_type))
        board = []
        ...
        for id_iter in range(setting.N_iter):
            ...
            for id_a in range(setting.N_agent):
                ...
                #     agent[id_a].refinement(board)
                # else:
                new_board = agent[id_a].dec_BP_conf_res(board, new_board)
                # print(agent[id_a].route)
            #     if agent[id_a].total_finish_flag:
            #         cnt += 1
            # if cnt == setting.N_agent and not fin_flg:
            #     fin_flg = True
                # print(id_iter)
                # print(id_a, agent[id_a].route)
        ...
        for i, a in enumerate(agent):
            ...
            for j in range(len(a.route)):
                if self.discount > 0:
                    r += self.task_reward[a.route[j]] * np.exp(-self.discount * (a.route_time[j] - self.task_time[a.route[j]]))
