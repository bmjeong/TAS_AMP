...
class BWCBBA:
    def __init__(self, prob):
        ...
        self.wbid_list = [[-1] * self.N_task for _ in range(self.N_agent)]
    ...
    def bundle_add(self, idx_agent: int):
        ...
        while not bundle_full_flag:
            ...
            array_logical_1 = ((np.array(self.wbid_list[idx_agent]) - np.array(self.winner_bid_list[idx_agent]))
                               > epsilon)
            # find the equal items
            array_logical_2 = (abs(np.array(self.wbid_list[idx_agent]) - np.array(self.winner_bid_list[idx_agent]))
                               <= epsilon)
            ...
            if value_max > 0:
                ...
                self.winner_bid_list[idx_agent][best_task] = self.wbid_list[idx_agent][best_task]
                ...
                self.scores_list[idx_agent].insert(best_indices[best_task], self.wbid_list[idx_agent][best_task])
    ...
    def compute_bid(self, idx_agent: int, feasibility: list):
        ...
        self.wbid_list[idx_agent] = [-1] * self.N_task
                ...
                if best_bid > 0:
                    ...
                    if bids:
                        if best_bid > min(bids):
                            self.wbid_list[idx_agent][idx_task] = min(bids)
                        else:
                            self.wbid_list[idx_agent][idx_task] = best_bid
                    else:
                        self.wbid_list[idx_agent][idx_task] = best_bid
